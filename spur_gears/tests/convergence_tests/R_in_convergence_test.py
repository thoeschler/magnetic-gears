...
def R_in_convergence_test():
    p1 = 14
    p2 = 14

    R_values = np.linspace(5., 20., num=8)
    ...
    for R1 in R_values:
        ...
        R_in_dir = f"R_in_convergence_{R1:.2f}"
        if not os.path.exists(R_in_dir):
            os.mkdir(R_in_dir)

        for D_ref in np.linspace(D - max(R1, R2), D + R1, num=50):
            ...
            d_angle_1 = (2 * np.random.rand(1).item() - 1) * np.pi / spur_gears.gear_1.p
            d_angle_2 = (2 * np.random.rand(1).item() - 1) * np.pi / spur_gears.gear_2.p

            spur_gears.remove_magnets(spur_gears.gear_1, D_ref=D_ref)
            spur_gears.remove_magnets(spur_gears.gear_2, D_ref=D_ref)

            print(f"""INFO
                {spur_gears.gear_1.p - len(spur_gears.gear_1.magnets)} out of {spur_gears.gear_1.p} magnets have been removed in gear 1 \n
                {spur_gears.gear_2.p - len(spur_gears.gear_2.magnets)} out of {spur_gears.gear_2.p} have been removed in gear 2 \n
            """)
            ...
            for e, name in zip(errors, names):
                with open(f"{R_in_dir}/{name}.csv", "a+") as f:
                    removed_magnets = (spur_gears.gear_1.p - len(spur_gears.gear_1.magnets)) / spur_gears.gear_1.p
                    f.write(f"{D_ref} {removed_magnets} {e} \n")

if __name__ == "__main__":
    R_in_convergence_test()