...
def test_ball_gear_mesh():
    ...
    mesh, cell_marker, facet_marker, magnet_subdomain_tags, magnet_boundary_subdomain_tags, \
        box_subdomain_tag = gear_mesh(ball_gear, x_M_ref=np.array([0., 20., 0.]), \
                                           mesh_size_space=3.0, mesh_size_magnets=0.4, \
                                            fname="ballgeartest", padding=r, write_to_pvd=True, \
                                                verbose=True)
...
def test_bar_gear_mesh():
    ...
    mesh, cell_marker, facet_marker, magnet_subdomain_tags, magnet_boundary_subdomain_tags, \
        box_subdomain_tag = gear_mesh(bar_gear, x_M_ref=np.array([0., 20., 0.]), \
                                           mesh_size_space=3.0, mesh_size_magnets=0.4, \
                                            fname="bargeartest", padding=h, write_to_pvd=True, \
                                                verbose=True)
...
def test_segment_gear_mesh():
    ...
    mesh, cell_marker, facet_marker, magnet_subdomain_tags, magnet_boundary_subdomain_tags, \
        box_subdomain_tag = gear_mesh(segment_gear, x_M_ref=np.array([0., 20., 0.]), \
                                           mesh_size_space=3.0, mesh_size_magnets=0.4, \
                                            fname="segmentgeartest", padding=w, write_to_pvd=True, \
                                                verbose=True)

def test_segment_mesh():
    _ = segment_mesh(10, 20, 4, np.pi / 4, np.zeros(3), np.array([0., 1., 1.]), \
                        "segmenttest", 1., write_to_pvd=True)
