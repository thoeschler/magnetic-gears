    ...
    def simulate(self, n_iterations=10):
        ...
        for _ in range(n_iterations):
            ...
            for j in range(n_iterations):
                B1 = self.interpolate_field_gear(self.gear_1, self.gear_2, "B", "CG", 1, 0.07, 4.0)
                B2 = self.interpolate_field_gear(self.gear_2, self.gear_1, "B", "CG", 1, 0.07, 4.0)
            ...
            mt = self.gear_1.magnet_type
            df.to_csv(self._main_dir + f"/sample_torque_{mt}.csv", mode="a+", index=False, header=False, sep="\t")
...
def sample_torque_ball(n_iterations):
    ...
    bg.mesh_gear(gear_1_ball, mesh_size_space=2.0, mesh_size_magnets=0.1, \
        fname="gear_1", padding=gear_1_ball.r / 2)
    bg.mesh_gear(gear_2_ball, mesh_size_space=2.0, mesh_size_magnets=0.1, \
        fname="gear_2", padding=gear_2_ball.r / 2)

    bg.simulate(n_iterations)
...
def sample_torque_bar(n_iterations):
    ...
    gear_1_bar = MagneticBarGear(n=12, h=2.0, w=0.75, d=0.5, R=12, x_M=np.zeros(3), axis=np.array([1., 0., 0.]))
    gear_2_bar = MagneticBarGear(n=15, h=2.0, w=0.75, d=0.5, R=18, x_M=np.array([0., 1., 0.]), axis=np.array([1., 0., 0.]))
    gear_1_bar.create_magnets(magnetization_strength=1.0)
    gear_2_bar.create_magnets(magnetization_strength=1.0)

    bg = CoaxialGears(gear_1_bar, gear_2_bar, D=35, main_dir=sample)
    bg.mesh_gear(gear_1_bar, mesh_size_space=2.0, mesh_size_magnets=0.1, \
        fname="gear_1", padding=gear_1_bar.h)
    bg.mesh_gear(gear_2_bar, mesh_size_space=2.0, mesh_size_magnets=0.1, \
        fname="gear_2", padding=gear_2_bar.h)
